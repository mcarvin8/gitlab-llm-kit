...
describe("gitlab pipelines API", () => {
  it("listPipelines passes encoded project path and optional query", async () => {
    ...
    expect(c.requestAllPages).toHaveBeenCalledWith(
      "/projects/group%2Fsub/pipelines",
      {
        ref: "main",
        per_page: 20,
      },
  ...
  it("getPipeline", async () => {
    ...
    expect(c.request).toHaveBeenCalledWith(
      "GET",
      "/projects/group%2Fsub/pipelines/9",
    );
  ...
  it("listPipelineJobs", async () => {
    ...
    expect(c.requestAllPages).toHaveBeenLastCalledWith(
      "/projects/p/pipelines/9/jobs",
      {},
    );
  });
  ...
  it("getJobTrace uses requestText for plain-text log", async () => {
    ...
    expect(c.requestText).toHaveBeenCalledWith(
      "GET",
      "/projects/p/jobs/40/trace",
    );
